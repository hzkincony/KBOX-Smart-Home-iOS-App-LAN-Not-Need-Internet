...
+(instancetype)SY_safeDictionaryWithObjects:(const id [])objects forKeys:(const id <NSCopying> [])keys count:(NSUInteger)cnt;
@end
...
+(instancetype)SY_safeDictionaryWithObjects:(const id [])objects forKeys:(const id <NSCopying> [])keys count:(NSUInteger)cnt {
    ...
    return [self SY_safeDictionaryWithObjects:safeObjects forKeys:safeKeys count:j];
}
@end
...
+(void)callSafeCategory
{
    ...
    LOG_Error
    [NSDictionary jr_swizzleClassMethod:@selector(dictionaryWithObjects:forKeys:count:) withClassMethod:@selector(SY_safeDictionaryWithObjects:forKeys:count:) error:&error];
    LOG_Error
